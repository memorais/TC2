...
Os estudos mais recentes mostram que a capacidade de compressão do formato \ingles{WebP} tem cerca de 39\% de ganho quando comparado com JPEG e JPEG2000, como pode-se observar na figura 1.1:
...
O desenvolvimento do codificador e do decodificador do formato WebP no \ingles{Mozilla Firefox} contribui com a popularização deste novo formato, visto que o Firefox está entre os navegadores mais utilizados atualmente e ainda não suporta nativamente o formato WebP.

De acordo com os estudos feitos pela Google, o WebP mostrou-se eficiente quando comparado com outros formatos mais popularmente utilizados na \ingles{web}. A utilização em larga escala do WebP, traz benefícios econômicos para empresas que possuem a necessidade de diminuir o volume total de dados trafegados.

Um \ingles{case} de adoção do novo formato, é o da própria \ingles{Google}, que substituiu as imagens, de formato PNGs e JPEGs para WebP, em sua \ingles{Web Store} do navegador Chrome, diminuindo a utilização de banda dirária, de acordo com o Gerente de Produto do Google, Stephen Konig \cite{ChromiumBlog}:
\begin{citacao}
...
O objetivo principal deste trabalho é integrar ao código nativo do Mozilla Firefox, o uso da biblioteca \ingles{libwebp} para que o navegador possa realizar a decodificação e codificação de imagens no formato WebP em tempo de execução, sem que seja necessário a utilização de plug-ins ou ferramentas adicionais.

A implementação do decodificador é fundamental para que o navegador consiga realizar a exibição de imagens que estiverem no formato WebP. Já a implementação do codificador, possibilita que \ingles{sites} escritos na linguagem HTML5 e que possuem a funcionalidade \ingles{canvas}, possam gerar gráficos no formato WebP.
...
A contribuição com o desenvolvimento de uma ferramenta Open Source se dá como objetivo secundário deste trabalho. O código implementado foi submetido à comunidade do Firefox e os \ingles{feedbacks} recebidos entrarão como proposta para trabalhos e revisões futuras.
...
No capítulo 2, a fundamentação teórica está exposta apresentando as tecnologias que possuem ligação direta com a implementação desenvolvida. Apresenta também a história da \ingles{World Wide Web} e comenta à respeito do seu futuro através da quinta e maior revisão da linguagem que a rege, o HTML. 
...
A figura 3.1 ilustra o formato simples com compressão \ingles{lossy} (a), o formato simples com a compressão \ingles{lossless} (b) e o formato estendido (c).

\begin{figure}[h]
  \centering
    \includegraphics[scale=0.5]{LossyLossLessExtended.png}
  \caption{Formato simples com compressão \ingles{lossy}, formato simples com a compressão \ingles{lossless} e formato estendido \cite{WebPContainer}}
\end{figure}
...
\begin{table}[ht]
	...
	\label{tbl:padc}}{
		...
		\begin{tabular}{|l|p{14cm}|}
			...
			\hline
			Libwebp		& Biblioteca do formato WebP utilizada para o processo de codificação e decodificação. \\ 
			\hline
		\end{tabular}
		}
\end{table}
...
Na figura 5.3 é possível ver um diagrama macro que mostra o nível de alteração necessária nos módulos de imagem e como eles se relacionam entre si.

\begin{figure}[h]
  \centering
    \includegraphics[scale=0.3]{ComponentesArquitetura.png}
  \caption{Diagrama dos componentes e suas relações}
\end{figure}

Também foram necessárias pequenas alterações em outros pontos, como por exemplo, a definição do \ingles{mime-type} para o formato WebP que foi acrescentada dentro do Firefox, mas estas não caracterizaram mudanças significativas, de forma que a maior parte do trabalho foi realizada em cima do núcleo de manipulação de imagens do navegador, conforme mostrado no diagrama dos componentes.
...
Na primeira fase o \ingles{autoconf} é invocado para que as características do sistema e do compilador utilizados possam ser determinadas. Aqui também é realizada toda verificação por dependências necessárias para realização do \ingles{build}. A segunda fase na verdade consiste na aplicação da configuração encontrada na primeira fase para todo o código fonte existente que será compilado. A terceira e última fase é quando ocorre efetivamente a compilação do código e a criação dos \ingles{links} entre os binários gerados, baseando-se nas configurações definidas nas fases anteriores. Ao final da terceira fase são executados os testes automatizados para validar a funcionalidade de cada item compilado. Desta forma o processo pode ser definido como uma árvore, caso qualquer uma das fases falhe, a próxima etapa não será iniciada.

Toda orquestração do processo de compilação é baseada no arquivo moz.build existente dentro de cada diretório que contém um código-fonte a ser compilado ou a ser exportado como referência (arquivos de cabeçalho por exemplo). A sintaxe de definição do arquivo moz.build obedece as regras regidas pela linguagem Python. Na figura 5.4 é possível observar como a hierarquia está montada e como cada software está envolvido dentro do processo. Como existem várias linguagens de programação existentes dentro do Firefox, a fase de preparação fica responsável por acionar o compilador correto para cada fonte.
...
\label{Decodificacao}
\begin{algorithmic}
	\STATE Execução do método InitInternal(\ingles{buffer})
	\STATE Execução do método WriteInternal
	\FOR{Para cada linha do \ingles{buffer}}
	\STATE Decodificar a linha do \ingles{buffer} WebP
...
Dos 13 métodos listados a interação com a libwebp foi criada dentro dos métodos StartImageEncode, AddImageFrame e EndImageEncode. Tudo começa no método InitFromData, ele é o primeiro método a ser chamado e tem como função receber o \ingles{buffer} do chamador. A partir daí, o método IniFromData executa o método StartImageEncode que é responsável por inicializar as estruturas de dados WebPPicture, WebPConfig e WebPMemoryWriter. O método AddImageFrame possui uma iteração que lê linha a linha do \ingles{buffer} e realiza a construção da codificação através do método WebPMemoryWrite da libwebp. Este método faz a gravação dos dados dentro da estrutura WebPPicture que é efetivamente a imagem no formato WebP. O último método chamado é o EndImageEncode que realiza a liberação da memória utilizada por todo o processo de codificação. O resumo desta rotina está descrita no algoritmo 5.2.

\begin{algorithm}
\caption{Processo de codificação}
\label{Codificacao}
\begin{algorithmic}
        \STATE Execução do método InitFromData(\ingles{buffer})
...
A implementação realizada e descrita neste trabalho permitiu ao Firefox codificar e decodificar o formato WebP com sucesso. Como pode ser observado na figura 5.7, está disposta uma página que possui uma imagem no formato WebP em um Firefox sem a implementação, consequentemente a imagem não é exibida. Já na figura 5.8 é possível verificar que a mesma imagem no formato WebP é exibida corretamente, dentro de um Firefox que possui a implementação do decodificador. 
...
Na figura 5.9 foi criado um HTML que utiliza o método \ingles{canvas} para desenhar uma figura no navegador. O método \ingles{toDataURL} é executado com o parâmetro \ingles{image/webp} para fazer com a imagem seja renderizada no formato WebP. Para este caso especificamente não há como mostrar uma diferença clara através de imagens, pois no Firefox sem a implementação do codificador, quando o método \ingles{toDataURL} é utilizado com o parâmetro \ingles{image/webp}, ele simplesmente realiza a renderização da imagem no formato PNG por não possuir o suporte ao WebP. 

Esta substituição de formatos na exibição ocorre conforme orientação no rascunho da especificação do elemento \ingles{canvas} para o HTML5 \cite{CanvasElement}:
\begin{citacao}
...
Como forma de avaliar o desempenho da solução de maneira prática, foi desenvolvido um \ingles{site} composto de imagens no formato PNG e JPEG, e um segundo site exatamente igual ao primeiro, apenas com as imagens convertidas de PNG e JPEG para WebP. O site completo em sua formação original possui tamanho total de 5953K, e após a conversão das imagens para WebP, o tamanho total passou a ser de 907K. A figura 5.10 apresenta a página inicial do \ingles{site} criado para teste.
...
Após a conversão de todos os arquivos de imagem para o formato WebP, os dois sites foram hospedados em um mesmo servidor \ingles{web}. Com isso, o tráfego de rede para diferentes rotas é evitado, e o tempo de resposta do servidor é basicamente o mesmo, não interferindo nas métricas de carregamento do \ingles{site}. 

A tabela 5.3 mostra uma comparação dos tempos para carregamento da página bem como o nível de utilização de \ingles{bytes} trafegados durante o processo de abertura da página.
...
Posteriormente, um segundo caso de teste foi feito, seguindo as mesmas premissas do teste anterior. Foi criado outro \ingles{site} para execução de casos de teste, porém com uma quantidade maior de imagens apresentadas na página inicial. Da mesma forma que o teste anterior, não houve navegação entre as páginas, e as métricas coletadas foram somente no acesso à página inicial, na qual contempla desde a solicitação até o carregamento total da página. 

O site completo em sua formação original possui tamanho total de 12452K, e após a conversão das imagens para WebP, o tamanho total passou a ser de 2107K. A figura 5.11 apresenta a página inicial do segundo \ingles{site} criado para teste.
...
A tabela 5.4 mostra uma comparação dos tempos para carregamento da página bem como o nível de utilização de \ingles{bytes} trafegados durante o processo de abertura da página do segundo caso de teste.
...
Mesmo com a evolução constante realizada pela equipe de desenvolvimento do WebP, existem ainda diversos processos burocráticos pendentes que atrapalham no processo de adoção da tecnologia, como por exemplo o cadastramento do formato dentro de órgãos de registro da internet como a IANA (\ingles{Internet Assigned Numbers Authority}) \cite{IANA}. Sabe-se que a adoção de um formato depende não somente da qualidade da tecnologia, mas sim da amplitude de sua utilização. Por este motivo o seu sucesso está diretamente associado à disseminação da tecnologia tanto por parte dos \ingles{softwares} de manipulação de imagens, quanto pelos navegadores de internet. 
...
A versão 0.3.0 da libwebp, utilizada no desenvolvimento do trabalho, oferece suporte à animação de imagens quadro a quadro, semelhante ao processo realizado pelo formato GIF. Entretanto este tipo de implementação não foi realizada no desenvolvimento do codificador e do decodificador, ficando como referência para trabalhos futuros que devem ser realizados. A criação de um \ingles{script} para realizar testes automatizados de funcionalidade para codificação e decodificação ao final do processo de compilação, também é uma necessidade para trabalhos futuros.

Com o \ingles{patch} elaborado dentro do trabalho submetido à comunidade, será preciso avaliar futuramente, os comentários dos desenvolvedores que serão dados com relação a implementação. Os \ingles{feedbacks} reportados dentro do chamado vão fazer parte de trabalhos futuros para evolução e possíveis ajustes da implementação.
