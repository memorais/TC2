...
O desenvolvimento do codificador e decodificador do formato \ingles{WebP} no \ingles{Mozilla Firefox}, trará de imediato a melhora no tempo de resposta na exibição das imagens dentro do navegador, visto que o tamanho dos arquivos no servidor será reduzido. Se considerar que mais de 60\% do tamanho total de um \ingles{website} são imagens, os benefícios podem ser substanciais. É importante ressaltar que a utilização em massa deste novo formato contribui na diminuição do trafego de rede na internet de maneira global, isso resulta diretamente em maior vazão para outros recursos que possuem um maior consumo de banda, como é o caso de um \ingles{streaming} de vídeo por exemplo.

Um \ingles{case} de utilização do novo formato, é o da própria \ingles{Google}, na qual substituiu as imagens, de formato PNGs e JPEGs para WebP, em sua \ingles{Web Store} do navegador Chrome. Diminuindo a utilização de banda dirária:
\begin{citacao}
	\ingles{By converting PNGs and JPEGs to WebP, the Chrome Web Store was able to reduce image sizes by about 30\% on averag. Given the number of requests Chrome Web Store serves, this adds up to several terabytes of savings every day. (Stephen Konig, Google Product Manager)}.
\end{citacao}

\section{Objetivo}

O objetivo principal deste trabalho é desenvolver uma solução na qual habilite o suporte de codificação e decodificação do formato WebP dentro do Mozilla Firefox. Embora a implementação do suporte ao \ingles{WebP} esteja somente disponível para dois navegadores (Chrome e Opera), estes constituem mais de 29\% de todos os navegadores utilizados dentro da internet hoje. A implementação do suporte ao \ingles{WebP} dentro do Mozilla Firefox aumentará este percentual para 52\%, já que cerca de 22\% dos navegadores utilizados hoje são Mozilla Firefox \cite{BrowserStats}.

O trabalho desenvolvido será ser entregue à comunidade do Firefox, mas sua aplicação oficial na árvore da Mozilla não é o alvo principal. O objetivo deste trabalho não visa a aceitação por parte da equipe do Mozilla, em integrar a implementação desenvolvida ao código fonte do repositório oficial do Firefox, mas sim em contribuir com o desenvolvimento da renderização nativa do formato WebP dentro do navegador.

\section{Organização do Texto}

Será o última coisa a ser escrita, pois depende da finalização dos demais capítulos.
%\begin{comment}
%Este trabalho está estruturado em seis capítulos. O primeiro capítulo apresenta uma introdução sobre o formato WebP, bem como a motivação e os objetivos deste %trabalho. Já no capítulo 2, a fundamentação teórica está exposta apresentando as tecnologias que possuem ligação direta com a implementação desenvolvida neste %trabalho. Alguns trabalhos relacionados ao assunto estão apresentados no quarto capítulo. E para finalizar, é descrita a solução proposta e os comentários finais, nos %capítulos 5 e 6, respectivamente.
%
%Este documento está estruturado em seis capítulos. O primeiro capítulo apresenta uma introdução inicial sobre o formato WebP, bem como a motivação e os objetivos %deste trabalho.
%No capítulo 2, a fundamentação teórica está exposta apresentando as tecnologias que possuem ligação direta com a implementação desenvolvida ao neste trabalho.
...
Com o objetivo de mostrar os ganhos do formato de imagem WebP com relação ao formato JPEG e o formato PNG, mostramos a seguir uma série de 3 pares de imagens diferentes, uma no seu formato original e a outra já convertida para o WebP e os detalhes com relação ao tamanho final e os recursos utilizados em cada um dos casos. As imagens foram retiradas aleatoriamente da internet e convertidas utilizando o conversor padrão fornecido juntamente com a libwebp.

O primeiro caso é de uma paisagem montanhosa cuja a largura é de 1150 pixels e a altura de 863 pixels. A imagem na figura X está no formato JPEG e seu tamanho original é de 430K. O processo de conversão desta imagem para WebP levou exatos 0,428 milissegundos em uma máquina com processador Intel Core I5 de 2.5GHz. Após a conversão para o formato WebP, como pode ser visto na figura Y, a imagem resultante foi reduzida para 228K, aproximadamente 46,98\% de ganho na compressão.
...
Como o primeiro caso retrata uma imagem com tamanho consideravelmente pequeno, fizemos uma avaliação com uma imagem maior para verificar se o comportamento do formato WebP é alterado. A imagem mostrada na figura Z está também no formato JPEG e possui largura de 4928 pixels e a altura de 3264 pixels. O tamanho original da imagem é de 6996K. O processo de conversão desta imagem para WebP levou 5098 milissegundos em uma máquina com processador Intel Core I5 de 2.5GHz. A imagem no formato WebP, como pode ser visto na figura V, foi reduzida para 1008K. Neste segundo caso o ganho foi maior ainda, uma redução de 85,59\% com relação a imagem original.
...
O último caso é uma imagem no formato PNG que utiliza o recurso de transparência (\ingles{alpha}), ela possui largura de 1353 pixels por 1034 pixels de altura. A imagem no formato original tem tamanho de 840K e o processo de conversão para o formato WebP levou 0,328 milissegundos em uma máquina com processador Intel Core I5 de 2.5GHz. A imagem resultante tem tamanho de 56K, uma redução de 93,33\% com relação ao tamanho original. O ganho maior neste caso pode ser explicado pelo próprio recurso de transparência que tem compressão mais eficiente no formato WebP, e pelo pequeno número de cores na imagem o que também acaba favorecendo os algoritmos do WebP. Todos os casos acima sofreram compressão do tipo \ingles{lossy}.
...
Em 2010 o engenheiro do Google Vikas Arora criou um \ingles{patch} para dar suporte apenas ao processo de decodificação do WebP dentro do Mozilla Firefox \cite{FirefoxBug}. Esse \ingles{patch} foi submetido à equipe de desenvolvimento com o objetivo de ser incluído oficialmente dentro do código fonte do Firefox. Entretanto, o \ingles{patch} não estava funcionando corretamente, haviam problemas no processo de compilação e ainda utilizava a versão 0.1 da libwebp, que ainda estava em fase inicial de desenvolvimento e possuía inúmeros erros. Esta primeira tentativa também utilizava diversas estruturas e padrões obsoletos que não deviam mais ser utilizadas dentro do Firefox, como por exemplo a utilização do tipo primitivo PRUint32 ao invés do uint32 que é do padrão C ANSI. Estes fatores levaram na não adoção da implementação criada por Vikas Arora.

Quando foi falado publicamente do nosso interesse em conduzir uma nova implementação, Vikas Arora, entrou em contato conosco e se colocou a disposição para auxiliar na correção do seu \ingles{patch} anterior ou na elaboração de um novo, também realizando a implementação do processo de codificação do formato WebP.

\chapter{Descrição do Projeto}

\section{Objetivos}

Conforme apresentado na seção 1.3, o objetivo deste trabalho é contribuir com o desenvolvimento de uma solução para codificação e decodificação do formato de imagem WebP para o browser Mozilla Firefox. 
Pretende-se, realizar em tempo de execução, a renderização de imagens do formato WebP através da utilização do Mozilla Firefox, possibilitando que o navegador realize a leitura e exibição deste formato de imagem para o usuário. %\begin{comment}Inicialmente será realizado um estudo da biblioteca do formato WebP, na qual é disponibilizada pelo Google. %\end{comment}
...
Resultados obtidos com a implementação realizada.
